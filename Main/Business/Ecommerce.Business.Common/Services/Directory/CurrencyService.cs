...
namespace Ecommerce.Business.Common.Services.Directory
{
    ...
    public class CurrencyService : ICurrencyService
    {
        ...
        public virtual async Task<double> ConvertToPrimaryExchangeRateCurrency(double amount, Currency sourceCurrencyCode)
        {
            ...
            if (exchangeRate == 0)
                throw new EcommerceException($"Exchange rate not found for currency [{sourceCurrencyCode.Name}]");
            result /= exchangeRate;
            return result;
        }
        ...
        public virtual async Task<double> ConvertFromPrimaryExchangeRateCurrency(double amount, Currency targetCurrencyCode)
        {
            ...
            if (exchangeRate == 0)
                throw new EcommerceException($"Exchange rate not found for currency [{targetCurrencyCode.Name}]");
